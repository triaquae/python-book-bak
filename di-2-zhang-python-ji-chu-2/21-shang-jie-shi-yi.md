...
其实上面的两种猜想都是对的。正常情况下字符串在内存里就是如我们猜想的第一种情况一养，每一次创建一个变量都会在内存中申请一块儿空间。

但是，python出于节省内存的角度思考，对于短字符串及一部分数字做出了优化\(\[-5,257\]\)，也就是说，对于这部分字符串和数字，python解释器会由于要定义的新变量内容与之前定义过的变量内容相同而不申请新的内存空间。
