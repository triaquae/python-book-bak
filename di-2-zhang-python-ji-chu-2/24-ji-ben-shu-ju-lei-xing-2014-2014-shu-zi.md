...
>>> a = 3.141592653513650912344701282563619315624237060546875
>>> a
3.141592653513651
>>> from decimal import *
>>> getcontext()
...
>>> getcontext().prec = 50
>>> a = Decimal(3.141592653513651054608317828332)
>>> a
Decimal('3.141592653513650912344701282563619315624237060546875')
```

```py
#不推荐：字符串格式化方式，可以显示，但是不准确，后面的数字没有意义。
>>> a = ("%.30f" % (1.0/3))  
>>> a  
...
要了解复数，还需要先了解虚数。虚数\(就是虚假不实的数\):平方为复数的数叫做虚数。
